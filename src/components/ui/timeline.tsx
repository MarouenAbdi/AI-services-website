    ...
    >
      {title && (
        <div className="max-w-7xl mx-auto py-12 px-4 md:px-8 lg:px-10">
          <h2 className="text-lg md:text-3xl mb-4 max-w-4xl">
            {title}
          </h2>
          {description && (
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-12 w-12 absolute left-1 md:left-1 rounded-full bg-card border border-white/[0.05] flex items-center justify-center">
                {item.icon}
              </div>
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-xl mb-3 text-left font-bold text-white">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
